...
if ($#argv < 1) then
	echo " "
	echo "*********************************"
	echo "@afni_proc_LOCALISER by AHB"$version
	echo "*********************************"
	echo " "
...
rm -Rf ${subj}.results # remove any results directory

echo "Generating afni_proc_script"
afni_proc.py -verb 2 -subj_id ${subj} -dsets ./rundata/${subj}_run*_ss+orig.HEAD \
