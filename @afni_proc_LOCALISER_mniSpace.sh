#!/bin/tcsh
# @afni_proc_LOCALISER_mniSpace.sh
...
if ($#argv < 1) then
	echo " "
	echo "*********************************************"
	echo "@afni_proc_LOCALISER_mniSpace by AHB"$version
	echo "*********************************************"
	echo " "
	...
	echo " "
	echo "INPUT: @afni_proc_LOCALISER_mniSpace.sh ..."
...
foreach run (`count $firstRunNum $lastRunNum -digits 3`)
	...
	if ( -f "./rundata/${subj}_run${run}_ss_mni+orig.HEAD" ) then
		echo "Files already exist - skipping "$run
	else
		if ( -f "./rundata/${subj}_run${run}_ss+orig.HEAD" ) then 
			echo "** Aligning to standard space, run "${run}
			align_epi_anat.py -anat ./rundata/mni_brain_skullstrip_shft+orig. -epi ./rundata/${subj}_run${run}_ss+orig. -epi_base 50 \
				-anat_has_skull no -epi_strip None -epi2anat -suffix _mni
			...
			rm -f ./rundata/tmp/${subj}_run${run}*+orig.*
...
rm -Rf ${subj}_mni.results # remove any results directory

rm -f ./rundata/mask_anat.${subj}
3dAutomask -prefix ./rundata/mask_anat.${subj} mni_brain_skullstrip+orig.

echo "Generating afni_proc_script"
afni_proc.py -verb 2 -subj_id ${subj}_mni -dsets ./rundata/${subj}_run*_ss_mni+orig.HEAD \
	-copy_files ./rundata/${subj}_run${baseRun}_ss_mni+orig.HEAD ./rundata/${subj}_run${baseRun}_ss_mni+orig.BRIK \
	-copy_anat ./mni_brain_skullstrip+orig \
#	-check_results_dir no \
	-bash \
	-blocks tcat despike align volreg blur mask scale regress \
	-outlier_count yes \
	-test_stim_files yes \
