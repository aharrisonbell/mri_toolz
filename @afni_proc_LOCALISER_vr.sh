...
if ($#argv < 1) then
	echo " "
	echo "*********************************"
	echo "@afni_proc_LOCALISER_vr by AHB"$version
	echo "*********************************"
	echo " "
	...
	echo "  -baseSession <SESSION ID>"
	echo "  -baseSessionRun <run number>"
	echo "	-tr <TR>"
...
set baseSession=$1
set baseSessionRun=${baseRun}
set baseSessionVolume=50
...
while ($cnt < $numarg)
	...
	case -baseSession:
		...
		breaksw
	case -baseSessionRun:
		set baseSessionRun=$2
		breaksw
	case -baseSessionVolume:
		set baseSessionVolume=$2
		breaksw
	case -baseRun:
		set baseRun=$2
		breaksw
...
# PART 1.2 : Prepare the Base Session (in case it hasn't been done already)
if ( -f "./rundata/${monkey_lc}_${baseSession}_run${baseSessionRun}_ss+orig.HEAD" ) then 
	echo "File already exist - skipping "$baseSessionRun
else
	rm -f ./rundata/${monkey_lc}_${baseSession}_run${baseSessionRun}*+orig* 
	
	echo "** Converting NIFTIs to AFNI format, run "${baseSessionRun}
	3dcalc -a ./rundata/${monkey_uc}_${baseSession}_run${baseSessionRun}.nii.gz -expr 'a' \
		-prefix ./rundata/tmp/${monkey_lc}_${baseSession}_run${baseSessionRun}_temp
	3dTcat -tr ${tr} -prefix ./rundata/${monkey_lc}_${baseSession}_run${baseSessionRun} \
		./rundata/tmp/${monkey_lc}_${baseSession}_run${baseSessionRun}_temp+orig.
	
	echo "** Skullstripping, run "${run}
		3dSkullStrip -input ./rundata/${monkey_lc}_${baseSession}_run${baseSessionRun}+orig. -monkey -orig_vol \
			-prefix ./rundata/${monkey_lc}_${baseSession}_run${baseSessionRun}_ss_mask
		3dcalc -a ./rundata/${monkey_lc}_${baseSession}_run${baseSessionRun}_ss_mask+orig. \
			-b ./rundata/${monkey_lc}_${baseSession}_run${baseSessionRun}+orig. \
			-expr '(a/a)*b' -prefix ./rundata/${monkey_lc}_${baseSession}_run${baseSessionRun}_ss+orig.
 		rm -f ./rundata/tmp/${monkey_lc}_${baseSession}_run${baseSessionRun}_temp+orig.*
endif

# PART 1.3 : Preprocessing
rm -f proc.${subj} # remove any pre-existing proc.* files
rm -Rf ${subj}_vr.results # remove any results directory

echo "Generating afni_proc_script"
afni_proc.py -verb 2 -subj_id ${subj}_vr -dsets ./rundata/${subj}_run*_ss+orig.HEAD \
	...
	-volreg_zpad 4 \
	-volreg_base_dset ./rundata/${monkey_lc}_${baseSession}_run${baseSessionRun}_ss+orig \
	-blur_size 3 \
	-mask_dilate 3 \
	-mask_apply epi \
	...
	-regress_stim_files \
		./stimfiles/${monkey_uc}_${session}_stim1_Faces.txt \
		./stimfiles/${monkey_uc}_${session}_stim2_Places.txt \
		./stimfiles/${monkey_uc}_${session}_stim3_Objects.txt \
		./stimfiles/${monkey_uc}_${session}_stim4_Scrambled.txt \
	-regress_opts_3dD \
		-jobs 4 \
		-num_glt 11 \
